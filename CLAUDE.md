...
## PandaDoc integration (agreements & e-signatures)
- **Purpose**: Generate agreement documents from PandaDoc templates and send contract contacts a request for e-signature, tracking signing status back onto the contract. Complements `ContractDocument` (uploaded PDFs + AI summary) — PandaDoc tracks the e-signature lifecycle and archives the signed PDF. Follows the standard vendor-integration shape (Config → Client → Service → webhook stack → thin controller → Settings UI).
- **PandaDocConfig** (`app/Support/PandaDocConfig.php`) — static helper. Settings: `pandadoc_api_key` (encrypted), `pandadoc_webhook_secret` (encrypted), `pandadoc_enabled`. Methods: `apiKey()`, `webhookSecret()`, `isEnabled()`, `isConfigured()`, `baseUrl()` (`https://api.pandadoc.com`).
- **PandaDocClient** (`app/Services/PandaDoc/PandaDocClient.php`) — Guzzle, `Authorization: API-Key {key}` header. Endpoints under `/public/v1`. Methods: `listTemplates()`, `createDocument()`, `getDocument()`, `sendDocument()`, `voidDocument()`, `downloadDocument()` (raw PDF bytes), `isHealthy()`. Testable-seam constructor (`?Client $http = null`) like TacticalClient; 429 backoff.
- **PandaDocStatus** enum (`app/Enums/PandaDocStatus.php`) — Draft/Sent/Viewed/Completed/Declined/Voided/Expired/Error. `fromApi()` normalizes PandaDoc's dotted `document.*` strings (and bare forms). `isTerminal()`, `isSigned()`, `label()`, `badgeClass()`.
- **PandaDocDocument** model (`pandadoc_documents` table, explicit `$table`) — one row per agreement: `contract_id`, `created_by`, `pandadoc_id` (unique), `name`, `status`, `template_id/name`, `recipient_email/name`, `signed_disk_path`, `metadata`, `sent_at`, `completed_at`. Soft deletes; force-delete removes the signed PDF from disk. `Contract::pandaDocuments()` HasMany; contract force-delete cascades force-delete.
- **PandaDocService** (`app/Services/PandaDoc/PandaDocService.php`) — `createFromTemplate()` (builds recipients + contract tokens `Contract.Name`/`Client.Name`/`Contract.StartDate`/`Contract.EndDate`, `metadata.psa_contract_id`), `send()`, `syncStatus()`, `handleWebhookEvent()`, `downloadSignedPdf()` (stores to `pandadoc-documents/{contract_id}/{pandadoc_id}.pdf`, non-fatal on failure), `delete()` (best-effort void then soft-delete). Client injected via constructor.
- **Webhook** — `POST /api/webhooks/pandadoc`. HMAC-SHA-256 verification via `VerifyPandaDocWebhookSignature` (hex digest of raw body keyed by `pandadoc_webhook_secret`, passed in the `signature` query param). Store-then-dispatch (mirrors QBO): `pandadoc_webhooks` table + `PandaDocWebhook` model + `ProcessPandaDocWebhook` queued job (`$tries=3`). Controller accepts an array of events (or a single event object) shaped `{event, data:{id, status}}`; the job reconciles the referenced document's status, downloading the signed PDF on completion.
- **UI** — `PandaDocDocumentController` (`app/Http/Controllers/Web/`, thin) at `/contracts/{contract}/pandadoc/...` (store/send/sync/download/destroy), all `abort_if` contract-mismatch. "Agreements (PandaDoc)" card in the contract detail sidebar (create-from-template form gated on `isConfigured()`, per-agreement send/refresh/download/delete). Settings > Integrations > PandaDoc card (Billing tab): API key, webhook shared key, test connection, enable toggle. Activity logged via `ContractActivity` (`pandadoc_created/sent/deleted`).
- **Logging**: `[PandaDoc]` prefix (`[PandaDoc Webhook]` for inbound events).
...
- **HuntressService** (`app/Services/Huntress/HuntressService.php`) — ticket creation from CW-format payload. Title parsing, client resolution via org mapping, dedup on `client_id` + subject hash (15-min window), asset linking by hostname. Post-remediation maps to Resolved (not Closed).
