...
- **THE TOOL LOOP DISPATCHES ONLY WHAT IT PUBLISHED (psa-ejzjd) — do not weaken this.** `executeToolLoop()` derives `$publishedTools = array_column($tools, 'name')` **once, before the round loop**, and refuses any `tool_use` whose name is not in it — *before* the executor is reached. **A published schema is a boundary, not documentation.** Dispatch is by name, so filtering a surface's schema without filtering its executor hardens nothing. This is the choke-point rule (`psa-mocr`) in practice: guard where the calls converge, not at each call site.
  - **What this seam covers, exactly — and what it does NOT.** The guard closes the **five lanes that pass through `AiClient`**: AI Technician (`psa-hbbuq`), deterministic triage (`psa-hryjm`), Teams (`psa-uw2o`), the staff Assistant (`psa-o8w6t` — four *write* arms were reachable, including an unscoped `propose_close`), and the portal chatbot. **It does NOT close staff MCP (`psa-vydpz`) or Chet.** Those never reach `AiClient` at all — `McpStaffController` constructs `AssistantToolExecutor` and calls `execute()` directly — so they sit on their own controller/executor boundary with their own grant gates and must be fixed there. The defect *class* spans both groups; this *guard* does not. Do not let the shared bead list blur that line.
  - **Fail-closed, not fail-hard.** An unpublished name returns a `tool_result` error to the model so the loop continues and the model can self-correct; it never throws, which would turn a hallucinated name into a failed turn. The refusal is phrased as *availability* ("not available in this deployment") because the common cause is benign prompt/schema drift — `Prompts.php` names vendor tool families unconditionally while the schema is config-gated — not an attack.
  - **Compute it ONCE, from `$tools` itself.** `$tools` is by-value and never mutated, so the allowlist cannot drift from what the model was offered. **Do not "refresh" it per round or re-derive it from a live source** (a config read, a vendor health probe): a second derivation taken moments apart is precisely the TOCTOU that cost `psa-uw2o` four review rounds.
  - **Per-surface guards STAY as defence in depth.** `TeamsReadOnlySurface` and `TechnicianAgentSurface` each derive their runnable set from their own delivered schema via `array_intersect`, making them equal-or-stricter than this check by construction. Do not delete them as "now redundant".
  - Adding a tool means adding it to the **published schema**, not just to an executor's match arms. An arm with no schema entry is dead code that reads like a feature.
...
- If a capability is structurally unavailable upstream, **hard-error with a message that says so** and names the working alternative. Do not return an empty result and let it read as an all-clear. `CippToolContract::unanswerable($toolName, $input)` is the pattern — a pre-flight refusal, consulted from `CippMcpToolRelay.php:142` and `HandlesCippTools.php:78`. (It answers from the *request*; a degraded *response* is caught downstream — see the queue guard below.)
...
- A "still loading" answer is not an empty one. `CippQueueGuard::assertNotQueueBacked()` throws when CIPP replies with a queue marker rather than data, and **both** clients call it — `CippMcpClient` (relay path) and `CippClient` (REST path, which also backs every CIPP sync service). Guarding one would have left the other unwrapping the same payload to `[]`. Its docblock cites the vendor producers for both shapes and records why `QueueId` is *not* a marker (the healthy branch emits it too). Latent today (every queue path is `AllTenants`-gated and we only send concrete tenants); the guard is what stops a future endpoint mapping from silently reintroducing the false-clear.
...
Its HTTP entrypoints — the functions that build the payloads our `CippMcpToolRelay`/`CippClient` receive — live under `Modules/CIPPHTTP/Public/Entrypoints/HTTP Functions/`, generally one `Invoke-<Endpoint>.ps1` per endpoint. Read the one you're integrating and follow it to its true producer (it may re-dispatch into `Modules/CIPPCore/`, e.g. `Get-GraphRequestList.ps1`). Then **cite the file:line in a comment next to your field list**, as the rule above requires, so the next person re-verifies instead of re-guessing.

**Search case-insensitively** — `find … -iname 'invoke-*.ps1'`, not `-name 'Invoke-*.ps1'`. At the time of writing 612 are capitalised and **2 are lowercase** (`invoke-DomainAnalyser_List.ps1`, `invoke-ListEmptyResults.ps1`), so a case-sensitive search on Linux silently misses them — and "silently misses a file" is how this whole class of bug starts. The casing trap this repo already documents for CIPP *payloads* applies to its *filenames* too.

Check whether a vendor is public before assuming either way — `curl -s https://api.github.com/repos/<owner>/<repo>` returns the repo JSON (`"private": false`) for a public repo. A **404 only tells you it is not publicly readable** — private and nonexistent are indistinguishable from the outside, so a 404 is not evidence the thing does not exist, only that you cannot get it this way. And don't guess the owner/repo to test against: a negative result about a URL you invented says nothing about the real artifact.
...
> **Historical note — a path that used to be here.** This section previously said to read specs from `~/repos/HaloClaude/docs/`, with a table pointing at per-vendor files for CIPP, Mesh, NinjaRMM, Todyl, Servosity and Zorus. **That directory does not exist on the dev box** (verified 2026-07-15). Where those files live now — if anywhere reachable — is *unknown*: ask Charlie before assuming they are gone or guessing what they contained. It is called out rather than silently deleted because the dead path actively caused the failure it was meant to prevent: an agent obeying "read the source" found nothing there and fell back to guessing, and at least one bead (psa-00s5) sat blocked on "no vendor source available" when the vendor itself was a 20-second public clone away.
