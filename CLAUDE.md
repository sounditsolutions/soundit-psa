...
- **THE TOOL LOOP DISPATCHES ONLY WHAT IT PUBLISHED (psa-ejzjd) — do not weaken this.** `executeToolLoop()` derives `$publishedTools = array_column($tools, 'name')` **once, before the round loop**, and refuses any `tool_use` whose name is not in it — *before* the executor is reached. **A published schema is a boundary, not documentation.** Dispatch is by name, so filtering a surface's schema without filtering its executor hardens nothing: the same defect was found and fixed **five separate times** (Teams `psa-uw2o`, AI Technician `psa-hbbuq`, deterministic triage `psa-hryjm`, staff MCP `psa-vydpz`, staff Assistant `psa-o8w6t` — the last with four *write* arms reachable, including an unscoped `propose_close`) before being closed once here, at the single seam all five lanes pass through. This is the choke-point rule (`psa-mocr`) in practice: guard where the calls converge, not at each call site.
