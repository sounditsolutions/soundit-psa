...
- **Graduated vs volume tiers — two rate cards, one seam, and a hard block** — The billing engine carries two *different, both legitimate* tier pricing models with deceptively similar shapes (each is an ascending `up_to` + `unit_price` list with an unbounded final entry). **Volume** (per-SKU `backup_storage_tiers`, backup-storage lines only): picks the one tier covering the measured quantity and bills the *whole* quantity at that single rate. **Graduated** (per-profile-line `pricing_tiers`, any quantity type): splits the quantity into bands and bills *each band* at its own rate. Same numbers, different money — 300 GB over 1.00/0.80/0.60 is **$240 volume** but **$260 graduated**. They are deliberately kept distinct: same shape with opposite semantics is exactly how you mis-bill a client, so do not "unify" them without also unifying the semantics.
  - **The combination is REFUSED, not silently resolved.** Real money must not hang on a code-precedence rule the operator never sees. `App\Support\PricingModelConflict` is the single predicate — a conflict needs all three of: graduated bands on the line, a volume rate card on its SKU, **and** `quantity_type = PerBackupStorageGb` (the only type that reads the card; a graduated per-user line on the same SKU overrides nothing and is not blocked). Every door that can create the conflict consults it: `RecurringProfileController::store()`/`update()` (validation error on `lines.N.pricing_tiers`), `RecurringProfileController::bulkAction()` `set_quantity_type` (refuses the whole action — flipping lines onto Backup Storage (GB) is what puts the SKU card in play), and `SkuController::syncBackupStorageTiers()` (the *other* direction: a SKU gaining volume tiers under an already-graduated line — the guard lives inside the writer, the only path that creates a card, so no caller can forget it). **Both escape hatches stay open:** clearing the SKU's tiers and turning graduated pricing off on the line are never themselves refused, or an operator with an existing conflict would be stranded.
  - **Runtime precedence survives as defence in depth.** Validation stops *new* conflicts; it cannot un-write a row that predates it. So `BillingService::priceLineSegments()` — the single seam where a (line, quantity) becomes money — still encodes **graduated (line) > volume (SKU) > flat** (a line-level setting beats one inherited from the product, the same precedence as `unit_cost_override ?? sku->unit_cost`), still logs a `[Billing]` warning naming the winner, and the profile show page renders a per-line rate-card badge plus an explicit "Graduated pricing wins" warning on a conflicting line — visible *before* Generate, not after.
