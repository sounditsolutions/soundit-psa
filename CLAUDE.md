...
- **Prepaid time expiration** — Hours-based prepay credits can expire. `PrepayService::expiryForCredit()` stamps each new credit's `expiry_date` from the contract's effective policy; `PrepayExpirationService` forfeits the unconsumed remainder of expired lots via idempotent FIFO replay (`prepay:expire` daily cron). Policy resolution mirrors the Empty Invoice Suppression precedent: `Contract::effectivePrepayExpiryMonths()` returns the per-contract `prepay_expiry_months` when non-null (**0 = never expire** — an explicit opt-out; a positive value = that many months), else inherits the global `prepay_default_expiry_months` setting (Settings → General; unset/0 = never). No backfill — applies only to credits added after the policy is set. Dollar-based prepay never expires. The expiry term is **staff-only** (deliberately excluded from the portal `updateAlertSettings` allowlist — a client must never set their own forfeiture term). The portal balance widget surfaces the next upcoming forfeiture via `PrepayExpirationService::nextExpiry()` (FIFO projection, "consume nothing more" upper bound).
