...
- **Overage quantity type** — Parameterized usage-based billing: `max(0, ceil((usage - base × included_per_base_unit) / overage_divisor))`. Profile line fields: `usage_license_type_id` (what to measure), `base_license_type_id` (what provides included count, optional — defaults to 1), `included_per_base_unit`, `overage_divisor`. Reuses `countLicensesByType()` for both usage and base lookups. SKU `included_per_unit` auto-fills the "Included" field for Overage lines in the UI. Works for any usage-based billing pattern: backup overage, email security seats, EDR agent overage, etc.
- **Formula-based quantity (all dynamic types)** — The `max(0, ceil((usage − base × included_per_base_unit) / overage_divisor))` formula generalizes beyond Overage to every dynamic quantity type (GitHub #126). Non-Overage types have no base license type, so `base` is 1: `included_per_base_unit` is a flat included allowance and `overage_divisor` bills in blocks. `resolveQuantity()` gets each type's raw count from `resolveRawUsage()`, then `applyQuantityFormula()` applies the shared formula. Neutral fields (included 0, divisor 1) return the raw count unchanged, so lines without a formula are unaffected. The profile-line form exposes Included/Divisor for every dynamic type (usage/base license selectors stay Overage-only); a one-time migration neutralizes pre-feature `included_per_base_unit` values that the old UI auto-fill could leave on non-Overage lines.
