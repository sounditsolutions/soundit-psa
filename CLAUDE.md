...
- **Custom quantity types** — Operator-defined asset-type counters that extend the built-in `PerWorkstation`/`PerServer` family to any asset category (firewalls, switches, printers, APs). `CustomQuantityType` model (`custom_quantity_types` table: `name`, `description`, `asset_types` JSON, `is_active`). `QuantityType::Custom` is the delegating enum case; a profile line with `quantity_type = custom` links to its definition via `custom_quantity_type_id` (FK, `restrictOnDelete` to protect billing). `BillingService::resolveQuantity()` resolves `Custom` through the shared `countAssets()` (contract-scoped when the contract has asset assignments, client-wide otherwise). Managed at Settings → General → "Manage custom quantity types" (`CustomQuantityTypeController`, `/settings/quantity-types`); asset-type checkboxes are sourced from distinct `assets.asset_type` values. Not offered as a SKU `default_quantity_type` (needs a per-line definition) nor in the profile bulk `set_quantity_type` action. Deletion is blocked while any profile line references it (deactivate instead).
