...
You need the PHP extensions CI installs — `mbstring`, `pdo_sqlite`, `sqlite3`, `bcmath`, `intl`, `gd`, `zip` — **plus `dom`, `libxml` and `curl`**, which CI's runner PHP already bundles but a stock distro build does not. On Debian/Ubuntu those are the `php8.3-xml` and `php8.3-curl` packages, separate from `php8.3-cli`; [`docs/INSTALL.md`](docs/INSTALL.md) installs them for the same reason. Dependencies declare all three (`soundasleep/html2text` and `tijsverkoyen/css-to-inline-styles` require `ext-dom`/`ext-libxml`, `cometbackup/comet-php-sdk` requires `ext-curl`), so without them the very first command above aborts on Composer's platform check — `soundasleep/html2text ... requires ext-dom * -> it is missing from your system` — rather than failing later. With them, that is the whole set for the SQLite path above; CI only ever runs SQLite. Add `pdo_mysql` too if you intend to follow [Demo data](#demo-data), which needs MariaDB; without it `php artisan migrate` fails with `could not find driver`. Nothing in the tree declares the database drivers, so a missing one shows up as a runtime error rather than a failed install.
