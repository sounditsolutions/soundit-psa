    ...
    private static function cometTools(): array
    {
        return [
            [
                ...
                'description' => 'Get one device\'s Comet backup posture: the LIVE outcome of its most recent backup job, last success/failure times, days since last success, and storage usage. job_state is the POSTURE of the most recent backup job — same vocabulary as comet_get_backup_posture: last_backup_succeeded / last_backup_failed / last_backup_running / last_backup_unknown (unrecognised vendor status — UNKNOWN, not passing, not a confirmed failure) / no_backup_jobs_observed (server answered with no backup jobs — backups may never have run) / unavailable (history could not be read: Comet server unreachable, or no synced Comet username). ONLY last_backup_succeeded means the newest backup worked — never relay any other value as healthy, and treat unavailable/unknown as UNKNOWN, not passing; absence of a failure is not evidence of success. jobs_checked_at stamps when the live read happened.',
                'input_schema' => [
                    'type' => 'object',
                    'properties' => [
                        'hostname' => [
            ...
            [
                ...
                'description' => 'Get recent backup job history for a device, plus its last successful and last failed backup across all time. Shows each job\'s outcome with failure subtype (error, timeout, quota, missed schedule, cancelled), type (backup/restore/retention), timestamps, duration, and sizes. Error message text is not available here — it lives in the Comet job log. job_state carries the device\'s backup POSTURE in the same vocabulary as comet_get_backup_status / comet_get_backup_posture (last_backup_succeeded / last_backup_failed / last_backup_running / last_backup_unknown / no_backup_jobs_observed) — when the history cannot be fetched the result carries job_state=unavailable with an error instead of a job list: treat backup state as UNKNOWN then, never as passing. job_state=no_backup_jobs_observed means the server really answered and reported no backup jobs at all (not an all-clear — backups may never have run); a 0 job_count only ever means the server really reported none.',
                'input_schema' => [
                    'type' => 'object',
                    'properties' => [
                        'hostname' => [
    ...
    public static function tacticalTools(): array
    {
        return [
            [
                ...
                'description' => 'Get comprehensive device info from Tactical RMM: status, hardware (CPU, RAM, disks), OS, platform, network (public/local IPs), logged-in user, uptime, reboot status, check summary. checks_coverage answers "is this device actually monitored?" separately from "is it healthy?": "none" = zero checks (UNMONITORED); "unverified" = checks exist but NONE currently passes (all failing or none reporting — broken/wrong-platform check or real incident; coverage cannot be demonstrated); only "verified" (an explicitly passing check) is coverage.',
                'input_schema' => [
                    'type' => 'object',
                    'properties' => [
            ...
            [
                ...
                'description' => 'Get all monitoring check results for a device: check name, type, status (passing/failing/pending), return code, last_run (null = never reported), and output, plus payload-level explicit counts (checks_passing/checks_failing/checks_not_reporting) and the checks_coverage verdict — verified requires at least one explicitly passing check. Script checks are annotated with platform_mismatch/platform_mismatch_reason when the attached script cannot run on the device platform (e.g. a Windows-only script on a Mac) — such a check fails on every run and is noise, not monitoring.',
                'input_schema' => [
                    'type' => 'object',
                    'properties' => [
                        'hostname' => ['type' => 'string', 'description' => 'Device hostname'],
