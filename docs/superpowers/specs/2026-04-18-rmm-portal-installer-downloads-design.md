...
2. **NinjaRMM:** Does the API expose installer URLs per org? Look for `/v2/organizations/{id}/installer` or similar in Ninja's OpenAPI spec. Fallback: if Ninja doesn't support this via API, the feature is unavailable for Ninja-mapped clients.
   > *Pointer corrected 2026-07-15 (psa-sslk):* this item originally said to check `~/repos/HaloClaude/docs/NinjaRMM/NinjaRMM-API-v2.json`. **That path does not exist on the dev box**, so following this item as written finds nothing — and the failure mode is then guessing the shape instead. Get the spec from Ninja's developer portal, or ask Charlie where that file lives now. See CLAUDE.md "API documentation references" for the method. Do not infer the endpoint's shape from this bullet.
