...
// innerHTML, so every value interpolated into it is an HTML sink. The values are
// not ours: the details rows and errorMessages are built from names and
// exception text that originate upstream (tenant records reached through CIPP),
// and the preview renders on a staff settings page. Any value going into that
// markup goes through esc() first.
//
// Six other endpoint-fed innerHTML sinks in this file are NOT swept here and are
// ticketed separately; do not read this helper's presence as file-wide coverage.
function esc(value) {
        ...
        if (data.error) {
            ...
            if (data.details && data.details.length > 0) {
                ...
                data.details.forEach(d => {
                    // actionBadge() returns markup this file wrote, so it is
                    // the one value here that must NOT be escaped.
