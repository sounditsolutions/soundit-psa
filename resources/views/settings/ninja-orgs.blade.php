...
@section('title', 'NinjaRMM Organization Mapping')

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-10">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h2 class="section-title mb-0">NinjaRMM Organization Mapping</h2>
        ...
        <p class="text-muted mb-3">
            Map NinjaRMM organizations to local clients. This enables device sync for each mapped client.
        </p>

        <form method="POST" action="{{ route('settings.ninja-orgs.update') }}">
            @csrf
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            @php
                                $ninjaId = $org['id'];
                                $mapped = $mappedClients->get($ninjaId);
                            @endphp
                            <tr>
                                <td>
                                    <strong>{{ $org['name'] ?? 'Unknown' }}</strong>
                                    <br><small class="text-muted">ID: {{ $ninjaId }}</small>
                                </td>
                                <td>
                                    <div class="client-search-wrapper" data-ninja-org-id="{{ $ninjaId }}">
                                        <input type="hidden"
                                               name="mappings[{{ $ninjaId }}]"
                                               value="{{ $mapped?->id }}"
