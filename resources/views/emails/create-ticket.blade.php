...
@section('title', 'Create Ticket from Email')

@section('content')
<div class="row mb-3">
    <div class="col">
        <a href="{{ route('emails.show', $email) }}" class="text-decoration-none text-muted">
...
<div class="row mb-3">
    <div class="col">
        <h4 class="section-title">Create Ticket from Email</h4>
    </div>
</div>
    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="col">
                ...
                @endif
                <small class="d-block text-muted">{{ $email->received_at?->toAppTz()->format('d M Y H:i') }}</small>
                <div class="mt-1">{{ $email->subject }}</div>
                @if($email->body_preview)
                    <small class="text-muted d-block mt-1">{{ Str::limit($email->body_preview, 200) }}</small>
                @endif
            </div>
        </div>
    </div>
...
<div class="card shadow-sm card-static">
    <div class="card-body">
        <form method="POST" action="{{ route('emails.store-ticket', $email) }}">
            @csrf
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="client_id" id="client_id" class="form-select @error('client_id') is-invalid @enderror" required>
                        ...
                        @foreach($clients as $c)
                            <option value="{{ $c->id }}" {{ old('client_id', $email->client_id) == $c->id ? 'selected' : '' }}>
                                {{ $c->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <x-markdown-editor name="description" id="description" rows="6" :value="$email->body_text ?? strip_tags($email->body_html ?? $email->body_preview ?? '')" />
                </div>
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-accent">
                    <i class="bi bi-ticket-perforated me-1"></i>Create Ticket & Link Email
                </button>
                <a href="{{ route('emails.show', $email) }}" class="btn btn-outline-secondary ms-2">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Pre-selected contact from email
    const preselectedContactId = '{{ old('contact_id', $email->person_id) }}';

    // Client change → load contacts and assets
    function loadClientData(clientId, preselectContact) {
    ...
    clientSelect.addEventListener('change', function() {
        loadClientData(this.value, null);
    });
    ...
    // Initial load: if client is pre-selected, load contacts/assets with preselected contact
    if (clientSelect.value) {
        loadClientData(clientSelect.value, preselectedContactId);
    }
