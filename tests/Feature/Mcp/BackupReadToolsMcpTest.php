...
class BackupReadToolsMcpTest extends TestCase
{
    ...
    public function test_servosity_backup_posture_roundtrips_over_the_mcp_boundary(): void
    {
        ...
        // objects, shadowprotect_keys as ShadowProtectKey objects). The
        // backup-jobs endpoint has NO documented response shape and is never
        // queried — there is deliberately no job mock to author. Fixtures
        // travel through the client's real identity-preserving decode so
        ...
        // The honesty contract delivered end-to-end: job-run state is the
        // constant UNVERIFIABLE block (the endpoint's response schema is
        // undocumented, so it is never queried and no count, zero, or
        // outcome is published), plus the canonical freshness trio and the
        // unverifiable provisioning plane.
        $this->assertSame('unverifiable', $result['job_run_history']['status']);
        $this->assertStringContainsString('UNVERIFIABLE', $result['job_run_history']['note']);
        ...
        $this->assertStringNotContainsString('records_observed', $payload, 'no job-record observation claim may cross the MCP boundary');
        $this->assertStringNotContainsString('unverified_record_count', $payload, 'no count-derived job field may cross the MCP boundary');
