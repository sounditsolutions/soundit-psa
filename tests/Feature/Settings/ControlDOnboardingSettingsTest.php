...
class ControlDOnboardingSettingsTest extends TestCase
{
    ...
    public function test_rejected_replacement_has_conditional_warning_and_aligned_profile_label(): void
    {
        Setting::setValue(ControlDConfig::CODE_EXPIRY_DAYS_SETTING, '-1');
        $this->save(['code_expiry_days' => '25', 'default_profile_id' => str_repeat('x', 65)])
            ->assertSessionHasErrors('default_profile_id');
        $html = $this->actingAs($this->user)->get(route('settings.integrations'))->assertOk()->getContent();
        $this->assertMatchesRegularExpression('/name="code_expiry_days"[^>]*value="25"/', $html);
        $this->assertStringContainsString('Leaving this field blank when', $html);
        $this->assertStringNotContainsString('this form will clear it', $html);
        $this->assertMatchesRegularExpression('/name="default_profile_id"[^>]*>\s*<div class="invalid-feedback">[^<]*Enforced profile ID/i', $html);
        $this->save(['code_expiry_days' => '25'])->assertSessionHasNoErrors();
        $this->assertSame(25, ControlDConfig::codeExpiryDays());
    }
